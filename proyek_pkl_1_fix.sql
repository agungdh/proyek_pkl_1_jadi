...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_prodi`  AS  select `p`.`id` AS `id`,`p`.`kode` AS `kode`,`p`.`nama` AS `nama`,`p`.`fakultas_id` AS `fakultas_id`,`f`.`kode` AS `kode_fakultas`,`f`.`nama` AS `nama_fakultas` from (`fakultas` `f` join `prodi` `p`) where (`p`.`fakultas_id` = `f`.`id`) ;
